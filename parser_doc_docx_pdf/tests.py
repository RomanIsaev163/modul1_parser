...
class TestParser(unittest.TestCase):
    def setUp(self):
        self.good_path_doc = 'test_files/test.doc'
        self.good_path_docx = 'test_files/test.docx'
        self.good_path_pdf = 'test_files/test.pdf'
    ...
    def test_content_doc_empty(self):
        content = self.doc_content_df[self.doc_content_df['doc_file_name'] == self.empty_doc_file_name]['text'].values[
            0]
        self.assertEqual('', content)

    def test_content_docx_empty(self):
        content = \
            self.docx_content_df[self.docx_content_df['doc_file_name'] == self.empty_docx_file_name]['text'].values[0]
        self.assertEqual('', content)

    def test_content_pdf_empty(self):
        content = self.pdf_content_df[self.pdf_content_df['doc_file_name'] == self.empty_pdf_file_name]['text'].values[
            0]
        self.assertEqual('', content)

    def test_content_rtf_empty(self):
        content = self.rtf_content_df[self.rtf_content_df['doc_file_name'] == self.empty_rtf_file_name]['text'].values[
            0]
        self.assertEqual('', content)
    ...
    def test_content_docx_all(self):
        content = self.docx_content_df[self.docx_content_df['doc_file_name'] == self.all_docx_file_name]['text'].values[
            0]
        self.assertEqual(self.all_symbols, content)
